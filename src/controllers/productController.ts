...
export const seedProducts = async (products: IProduct[]) => {
  try {
    await Product.insertMany(products);
  } catch (err) {
    console.log({ msg: "Error seeding products" });
  }
};
...
export const listStocks = async (req: Request, res: Response) => {
  try {
    ...
    const filtered = data
      ...
      .filter((stock: any) => ACTIVE_US_TICKERS!.includes(stock.symbol))
      // .filter((stock: any) => stock.type === "Common Stock")
      .slice(0, 20);

    seedProducts(filtered);
    res
      .status(200)
